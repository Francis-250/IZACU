          ...
          <div className="grid md:grid-cols-6 sm:grid-cols-3 grid-cols-2 gap-4">
            {data.slice(0, 6).map((content) => (
              <Link
                to={`/movie/${content.id}`}
                key={content.id}
                className="relative rounded-lg overflow-hidden group cursor-pointer min-h-[250px] bg-cover bg-center"
                style={{
                  backgroundImage: `url("https://drive.google.com/uc?export=view&id=${content.thumbnail_url}")`,
                }}
              >
